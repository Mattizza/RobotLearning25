...
def train(env_name, print_things=True, train_run_id=0, train_episodes=1000,
    ...
    for episode_number in range(train_episodes):
        ...
        if episode_number % 1_000 == 0:
            print('episode_number:', episode_number, 'run_id:', train_run_id)
        ...
        agent.episode_finished(episode_number, mode)
    ...
    # with open(f'{path}/data_{time.time()}.pkl', 'wb') as f:
    #             pkl.dump(data, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="naive", help="Typology of the policy gradient")
    parser.add_argument("--path", type=str, default=None, help="Path the data")
    ...
    if args.test is None:
        try:
            data = train(args.env, train_episodes=args.train_episodes)
            # with open(f'{args.path}/data.pkl', 'wb') as f:
            #     pkl.dump(data, f)
